...
class Events extends Component {
  ...
  viewEventStatus = (id, registrationStatus) => {
        ...
        <ul className="events-list">
          {eventsList.map(eachEvent => (
            <EventItem
              key={eachEvent.id}
              eventItem={eachEvent}
              viewEventStatus={this.viewEventStatus}
              isEventActive={eachEvent.id === activeEventId}
            />
          ))}
        </ul>
      </div>
  ...
  renderActiveEventRegistrationDetails = () => {
    ...
    return (
      <div className="registration-status-body">
